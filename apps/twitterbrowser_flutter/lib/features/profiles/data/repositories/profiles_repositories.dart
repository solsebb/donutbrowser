...
class HostedProfilesRepository implements ProfilesRepository {
  HostedProfilesRepository(this.authService);

  final HostedAuthService authService;

  ProfilesRepositoryException _mapHostedFetchException(Object error) {
    if (error is ProfilesRepositoryException) {
      return error;
    }
    if (error is HostedAuthException) {
      return ProfilesRepositoryException(error.message);
    }
    if (error is http.ClientException) {
      return const ProfilesRepositoryException(
  ...
  Future<List<BrowserProfileSummary>> fetchProfiles() async {
    try {
      final response = await authService.fetchHostedProfiles();

      if (response.statusCode == 403) {
        throw const ProfilesRepositoryException(
          'Hosted sync is not enabled for this account yet. Open desktop TwitterBrowser, sign into this same account, and enable hosted sync there first.',
        );
      }

      if (response.statusCode != 200) {
        throw const ProfilesRepositoryException(
          'Unable to load hosted profiles from the hosted catalog.',
        );
      }
      ...
      throw _mapHostedFetchException(error);
    }
  }
  ...
  Future<BrowserProfileSummary> fetchProfile(String id) async {
    try {
      final response = await authService.fetchHostedProfiles(profileId: id);
