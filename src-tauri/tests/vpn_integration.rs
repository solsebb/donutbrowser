...
async fn test_wireguard_tunnel_init() {
  ...
  use twitterbrowser_lib::vpn::{VpnTunnel, WireGuardTunnel};
...
async fn test_openvpn_tunnel_init() {
  ...
  use twitterbrowser_lib::vpn::{OpenVpnTunnel, VpnTunnel};
...
async fn test_tunnel_manager() {
  use twitterbrowser_lib::vpn::{TunnelManager, VpnStatus, VpnTunnel};
  ...
  impl VpnTunnel for MockTunnel {
    async fn connect(&mut self) -> Result<(), twitterbrowser_lib::vpn::VpnError> {
      self.connected = true;
      Ok(())
    }

    async fn disconnect(&mut self) -> Result<(), twitterbrowser_lib::vpn::VpnError> {
